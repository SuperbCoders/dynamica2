...
RSpec.describe Stats::Forecast, type: :model do
  ...
  describe '#series_quantities' do
    ...
    context 'by days' do
      subject { Stats::Forecast.new(item, period: :day, from: Time.parse('01-01-2014'), to: Time.parse('05-01-2014')) }
    ...
    context 'by months' do
      subject { Stats::Forecast.new(item, period: :month, from: Time.parse('01-01-2014'), to: Time.parse('01-03-2014')) }
      before { StatsHelper.generate_values(5, item: item) }
      it 'generates list of dates for time series' do
  ...
  describe '#calculate' do
    ...
    context 'by days' do
      subject { Stats::Forecast.new(item, period: :day, depth: 3, from: Time.parse('01-01-2014'), to: Time.parse('05-01-2014')) }
      before { StatsHelper.generate_values(5, item: item) }
    ...
    context 'by month' do
      subject { Stats::Forecast.new(item, period: :month, depth: 1, from: Time.parse('01-01-2014'), to: Time.parse('01-03-2014')) }
      before { StatsHelper.generate_values(5, item: item) }
      it 'generates list of predictions' do
